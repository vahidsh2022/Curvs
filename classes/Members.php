...
class SAP_Members{
		...
		if ( !sap_current_user_can('members') ) {
			...
			if ( !empty($_SESSION['user_details']) ) {
				...
				foreach ( $result as $member ) {
					...
					$name = '<a href="' . $router->generate('profile_member',['id'=>$member->id]) . '">'.$member->first_name . ' ' . $member->last_name;
					$status = $member->status == '1' ? '<div class="plan-active">Active</div>':'<div class="plan-inactive">In-active</div>';
					$action_links = '<a href="'.SAP_SITE_URL . '/member/edit/' . $member->id.'" data-toggle="tooltip" data-placement="top" title="Edit"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>';

					if( $curUserId != $member->id ) {
						$action_links .= ' <a class="delete_member" id="member_'.$member->id.'" data-toggle="tooltip" data-placement="top" title="Delete" aria-data-id="'.$member->id.'"><i class="fa fa-trash" aria-hidden="true"></i></a>';
					}
	...
	public function profile_member() {
		if ( !sap_current_user_can('plans') && !empty($_SESSION['user_details']) ) {			
			include_once( $this->common->get_template_path('Members' . DS . 'profile.php' ) );
		}
		else {
			$this->common->redirect('login');
		}
	...
	public function your_subscription() {
		...
		if (isset($_SESSION['user_details']) && !empty($_SESSION['user_details'])) {
			...
			$subscription_details 	=  $this->membership->get_membership_data_by_user_id($user_details['id'],true );	
            // $subscription_details 	= $this->user->get_user_subscription_details($user_details['id']);
			// $subscription_details 	= $this->user->get_membership_data_by_user_id($user_details['id']);
