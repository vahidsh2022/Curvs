...
class SAP_Quick_Posts
{
	...
	{
		if (sap_current_user_can('quick-posts-users')) {
			$this->common->redirect('login');
		}

		include_once $this->common->get_template_path('Quick-Posts' . DS . 'users.php');
	}

	public function detail()
	{
		$userWhere = sap_get_current_user_role() == 'user' ? 'and user_id = ' . sap_get_current_user_id() : '';

		if (empty($this->_id)) {
			http_response_code(400);
			die(json_encode(['status' => 'error', 'messages' => ['id' => 'requierd']]));
		}
		$post = $this->db->get_results("select * from $this->table_name where post_id = $this->_id $userWhere")[0] ?? new stdClass;


		die(json_encode([
			'message' => ['title' => lang('message'), 'value' => ($post->message ?? '-')],
			'image' => ['title' => lang('image'), 'value' => empty($post->image) ? '/assets/images/no-imag.png' : '../uploads/' . $post->image],
			'video' => ['title' => lang('video'), 'value' => empty($post->video) ? '' : '../uploads/' . $post->video],
			'created_date' => ['title' => lang('created_date'), 'value' => $post->created_date],
			'status' => ['title' => lang('status'), 'value' => lang($post->status == 1 ? 'published' : 'scheduled')],
		]));
	}
	...
	public function add()
	{
		...
		include_once $this->common->get_template_path('Quick-Posts' . DS . 'add.php');
	}

	public function add_from_cpg()
	{
		if (!sap_current_user_can('quick-post')) {
			$this->common->redirect('login');
			return false;
		...
		try {
			global $match;
			$this->cpgRow = $this->getCpgById($match['params']['cpg_id']);
		} catch (Exception $exception) {
			$this->flash->setFlash("Cpg does not belongs to you", 'error');
	...
	public function save_post()
	{
		...
		if (true) {
			...
			if (!empty($_FILES['video']['name'])) {
				...
				if ($_POST['networks']['telegram']) {
					...
					if ($size_conversion <= 10) {
						...
						$this->flash->setFlash('Limit for video size in Telegram Posting is maximum 10MB', 'error');
						$validate_size = 0;
					}
				}
			...
			if (!empty($_POST['sap-schedule-time']) && strtotime($_POST['sap-schedule-time']) > strtotime(date('Y-m-d H:i:s'))) {
				...
				if (
					(!empty($_POST['sap-schedule-time-fb']) && strtotime($_POST['sap-schedule-time-fb']) > strtotime(date('Y-m-d H:i:s'))) ||
					...
					(!empty($_POST['sap-schedule-time-tg']) && strtotime($_POST['sap-schedule-time-tg']) > strtotime(date('Y-m-d H:i:s'))) ||
					...
					(!empty($_POST['sap-schedule-time-wordpress']) && strtotime($_POST['sap-schedule-time-wordpress']) > strtotime(date('Y-m-d H:i:s')))
				) {
					$_POST['status'] = 2;
				}
				...
				if (
					(empty($_POST['sap-schedule-time']) && empty($_POST['sap-schedule-time-fb']) && $_POST['networks']['facebook']) ||
					...
					(empty($_POST['sap-schedule-time']) && empty($_POST['sap-schedule-time-tg']) && $_POST['networks']['telegram']) ||
					...
					(empty($_POST['sap-schedule-time']) && empty($_POST['sap-schedule-time-wordpress']) && $_POST['networks']['wordpress'])
				) {
...
//            if(isset($_POST['networks']['telegram']) && !empty($_POST['networks']['telegram'])) {
//                if (!class_exists('SAP_telegram')) {
//                    require_once(CLASS_PATH . 'Social' . DS . 'telegramConfig.php');
//                }
//                $sapTelegram = new SAP_Telegram();
//                $selectedAccounts = $sapTelegram->getSelectedAccounts($_POST['networks'],$user_id);
//
...
//                    $this->flash->setFlash("Teelgram keys not found in user settings.",'error');
...
//                }
//                $prepare_data['message'] = $this->CPG->messageWithFooter($prepare_data['message'],$selectedAccounts['name'],'',$keys['telegram_keys'],'');
//            }
	...
	public function update_post($prepare_data = array(), $where_data = array())
	{
		...
		if (isset($_POST['quick-form-updated'])) {
			...
			if (!empty($_FILES['video']['name'])) {
                ...
                $_POST['video'] = $post->video;
                $_POST['image'] = $post->image;
                $_POST['media'] = $post->media;
            }
	...
	public function get_all_posts()
	{
		$result = array();
		try {
			$result = $this->db->get_results(<<<sql4QP
		SELECT 
		qp.* , ur.first_name, ur.last_name, pm.meta_key, 
		MAX(CASE WHEN pm.meta_key = 'sap_schedule_time' THEN pm.meta_value END) AS schedule,
		MAX(CASE WHEN pm.meta_key = 'sap_networks' THEN pm.meta_value END) AS networks
		FROM sap_quick_posts as qp 
		left join sap_users as ur on qp.user_id = ur.id 
		left join sap_quick_postmeta as pm on qp.post_id = pm.post_id 
		group by qp.post_id 
	...
	public function save_post_meta()
	{
		...
		//Preapare Data telegram blogger time
		if (!empty($_POST['status']) && $_POST['status'] == 2 && !empty($_POST['sap-schedule-time-tg'])) {
			$this->update_post_meta($_POST['id'], 'sap_schedule_time_tg', strtotime($_POST['sap-schedule-time-tg']));
		}
		...
		//Preapare Data reddit blogger time
		if (!empty($_POST['status']) && $_POST['status'] == 2 && !empty($_POST['sap-schedule-time-wordpress'])) {
			$this->update_post_meta($_POST['id'], 'sap_schedule_time_wordpress', strtotime($_POST['sap-schedule-time-wordpress']));
		}
	}
	...
	public function sap_manage_wall_social_post($post_id, $scheduled = false, $user_id = '', $individual_post = '')
	{
		...
		//Get general options of Telegram;
		$sap_telegram_options = array();

		if (in_array('telegram', $networks)) {
			$sap_telegram_options = $this->settings->get_user_setting('sap_telegram_options', $user_id);
		...
		if (in_array('wordpress', $networks)) {
			$sap_wordpress_options = $this->settings->get_user_setting('sap_blogger_wordpress_data', $user_id);
		...
		$wordpress_status = $this->get_post_meta($post_id, 'wordpress_status');
		$sap_schedule_time_wordpress = $this->get_post_meta($post_id, 'sap_schedule_time_wordpress');
		//Check wordpress enable
		if (!empty($sap_networks_meta['wordpress'])) {
			// if wordpress is enable and sap_fb_status is empty or 2 then go inside if condition
			if ($wordpress_status != '1' && $wordpress_status != '3') {
				//Check schedule enable
				if (!empty($_POST['status']) && $_POST['status'] == 2 && $scheduled == 2) {
					$this->update_post_meta($post_id, 'wordpress_status', 2);
					$this->flash->setFlash($this->sap_common->lang('quick_post_wordpress'), 'success');
				} else {

					// if individual schedule time is not set and individual_post  parameter is false then post with global time
					// if individual schedule time is set and individual_post parameter is fb given then post with individual time
					if ((empty($sap_schedule_time_wordpress) && $individual_post == false) || $individual_post == 'wordpress') {

						if (!class_exists('SAP_Wordpress_Config')) {
							require_once(CLASS_PATH . 'Social' . DS . 'wordpressConfig.php');
						}

						$wordpress_config = new SAP_Wordpress_Config($user_id);

						$wordpress_result = $wordpress_config->sap_quick_post_on_wordpress_post($post_id);

						if (!empty($wordpress_result)) {
							$this->update_post_meta($post_id, 'wordpress_status', '1');
						} else {
							// '3' for tried to published when schedule time occur but error occur  
							$this->update_post_meta($post_id, 'wordpress_status', '3');
						}
					}
				}
			}
		...
		$schedule_time_telegram = $this->get_post_meta($post_id, 'sap_schedule_time_tg');
		$telegram_status = $this->get_post_meta($post_id, '_sap_tg_status');
		//Check telegram enable
		if (!empty($sap_networks_meta['telegram'])) {
			if ($telegram_status != '1' && $telegram_status != '3' && $telegram_status != '4') {
				//Check schedule enable
				if (!empty($_POST['status']) && $_POST['status'] == 2 && $scheduled == 2) {
					$this->update_post_meta($post_id, '_sap_tg_status', 2);
					$this->flash->setFlash(lang('quick_post_telegram'), 'success');
				} else {
					if ((empty($schedule_time_telegram) && $individual_post == false) || $individual_post == 'telegram') {
						//Upload custom image if exist
						if (!class_exists('SAP_telegram')) {
							require_once(CLASS_PATH . 'Social' . DS . 'telegramConfig.php');
						}

						$this->tgposting = new SAP_telegram($user_id);
						$tg_result = $this->tgposting->sap_quick_post_to_telegram($post_id);

						if (!empty($tg_result)) {
							$this->update_post_meta($post_id, '_sap_tg_status', '1');
						} else {
							// '3' for tried to published when schedule time occur but error occur  
							$this->update_post_meta($post_id, '_sap_tg_status', '3');
						}
					}
				}
			}
	...
	{
		if (!sap_current_user_can('quick_posts')) {
			$this->common->redirect('login');
			return false;
		}
		include_once $this->common->get_template_path('Quick-Posts' . DS . 'history.php');
