...
class SAP_Mingle_Update {
    ...
    public function check_update() {
        ///> prevent auto update.
        ///> for improve load pages in dev time.
        return '5.5.2';
    ...
    public function version_updating() {
        ...
        //Verify the license during plugin updates. If the verification fails, the license will be removed.
        $sap_license_activation = 'Verify License';
        $data = $this->wpw_auto_poster_render_activation_settings( $license_data['license_key'], $license_data['license_email'], $sap_license_activation );
        if ( isset( $data['status'] ) && 1 != $data['status'] ) { 
            $response['error'] = $data['msg'];
            $this->settings->delete_options('sap_license_data');
            ...
            if(isset($_POST['sap_register_license'])){
                ...
                if ( $status == 'true' ) {
                    ...
                    if(isset($data) && $data) {
                        ...
                        $this->flash->setFlash( $status_msg = $this->sap_common->lang('something_went_wrong'), 'error');
                    }
                }
    ...
    public function wpw_auto_poster_render_activation_settings( $sap_license_key, $sap_license_email, $sap_license_activation = null, $sap_access_token = null, $access_token_action = null ) {
        ...
        if (isset($_SESSION['Update_version']) && !empty($_SESSION['Update_version'])) {
            $version = $_SESSION['Update_version'];
        }else{
            $version = $this->settings->get_options('sap_version');
        }
