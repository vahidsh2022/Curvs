...
class SAP_Gmb {
    ...
    public function sap_send_quick_post_to_gmb($post_id) {
        ...
        $sap_gmb_sess_data = !empty($this->settings->get_user_setting('sap_google_business_sess_data', $user_id)) ? $this->settings->get_user_setting('sap_google_business_sess_data', $user_id) : array();
        ...
        if (!empty($sap_gmb_sess_data)) {
            ...
            if (!empty($title))
                $title = $this->quick_posts->sap_limit_character($title, 1500);
