	...
	public function sap_check_time_and_published_post($post_id, $user_id, $is_quick_post)
	{
		...
		if ($is_quick_post) {
			...
			$status_name_meta = array(
				...
				'instagram' => '_sap_instagram_status',
				'reddit' => '_sap_reddit_status',
				'wordpress' => '_sap_wordpress_status'
			);
		}
		...
		$sap_schedule_time_telegram = $sap_fun->get_post_meta($post_id, 'sap_schedule_time_tg');
		...
		$sap_schedule_time_wordpress = $sap_fun->get_post_meta($post_id, 'sap_schedule_time_wordpress');
	

		//check global time
		if ($this->now || !empty($schedule_time) && $schedule_time < time()) { // check post schedule time based on user time zone
		...
		//check telegram individual time
		if (!empty($sap_schedule_time_telegram) && $sap_schedule_time_telegram < time()) { // check post schedule time based on user time zone
			//Manage wall posting of Socials
			$sap_fun->sap_manage_wall_social_post($post_id, 1, $user_id, 'telegram');
		}
		...
		//check reddit individual time
		if (!empty($sap_schedule_time_wordpress) && $sap_schedule_time_wordpress < time()) { // check post schedule time based on user time zone

			//Manage wall posting of Socials
			$sap_fun->sap_manage_wall_social_post($post_id, 1, $user_id, 'wordpress');
		}
		...
		$telegram_status = $sap_fun->get_post_meta($post_id, $status_name_meta['telegram']);
		if (!empty($telegram_status)) {
			$published['telegram_status'] = $telegram_status;
		}
		...
		$wordpress_status = $sap_fun->get_post_meta($post_id, $status_name_meta['wordpress']);
		if (!empty($wordpress_status)) {
			$published['wordpress_status'] = $wordpress_status;
		}
