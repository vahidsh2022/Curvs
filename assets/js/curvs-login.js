...
$(document).ready(function () {
		...
		$( '#add-member').validate( {			
			...
			errorPlacement: function (error, element) {
				...
				if (element.attr("name") === "terms_and_conditions") {
					error.insertAfter(element.closest(".terms-and-conditions"));
				} else {
			...
			$.ajax({
				...
				success: function (response) {
					...
					if(response.status == 200) {
						alertType = 'success';
						var disc_amt = 0;
						var main_amt = $('.rcp_main_total').val();
						if(parseFloat(main_amt) > 0) {
							if(response.data.coupon_type != "fixed_discount") {
								disc_amt = (parseFloat(main_amt) * parseFloat(response.data.coupon_amount)) / 100;
							} else {
								disc_amt = parseFloat(response.data.coupon_amount);
							}
							var payable_amt = 0;
							if(parseFloat(main_amt) > disc_amt) {
								payable_amt = parseFloat(main_amt) - disc_amt;
							}
	...
	$("label[for='terms_and_conditions']").on("click", function() {
		const checkbox = $("#terms_and_conditions");
		checkbox.prop("checked", true); // تغییر وضعیت چک‌باکس
		$("#add-member").validate().element(checkbox); // اعتبارسنجی دستی
	});
});