...
class FileUploader {
    ...
    public function uploadFile($_file_filed_name = FALSE, $file_name = '' ) {
        ...
        $this->_file_name = ($file_name) ? $file_name : strip_tags($_FILES[$this->_file_filed_name]['name']);
    ...
    public function uploadFileFromUrl(string $url)
    {
        // دریافت محتوای عکس از URL
        $fileData = file_get_contents($url);

        if ($fileData === false) {
            return false; // اگر دریافت عکس با خطا مواجه شد
        }
        ...
        if (!isset($mimeToExt[$mimeType])) {
            return false; // اگر نوع فایل نامعتبر بود
        }

        $extension = $mimeToExt[$mimeType];

        // ذخیره‌سازی عکس در فولدر مورد نظر

        $fileName = uniqid() . '.' . $extension;
        $filePath = SAP_APP_PATH . DS . $this->_config['file_upload_dir'] . DS . $fileName;

        $result = file_put_contents($filePath, $fileData);

        if ($result === false) {
            return false; // اگر ذخیره‌سازی با خطا مواجه شد
        }

        return $fileName; // اگر عملیات موفقیت‌آمیز بود
    }

    public function uploadFileFromFile($media)
    {
        ...
        // دریافت MIME type
        $finfo = new finfo(FILEINFO_MIME_TYPE);
        $mimeType = $finfo->file($media['tmp_name']);
        ...
        // انتقال فایل
        $result = move_uploaded_file($media['tmp_name'], $filePath);
